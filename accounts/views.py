...
def forgot_password(request):
    if request.method == 'POST':
        ...
        # print(url)
        message_type, message_content = send_email(
            to_email=email, url=url, fullname=user_profile.full_name, mode='forgot_password')
    ...
    if request.method == 'POST':
        try:
            ...
            email = get_object_or_404(
                ForgotPasswordConfirmation, id=forgot_password_confirm_id).email
...
def log_in(request):
    ...
    if request.method == 'GET':
        ...
        if request.GET.get('forgot_password') == 'Yes':
            forgot_password_confirm = get_object_or_404(
                ForgotPasswordConfirmation, id=request.GET.get('id'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if t_user.exists():
                user = authenticate(
                    username=t_user[0].username, password=password)
                print('user---->', user)
                if user is not None:
                    if save_password:
...
def register(request):
    ...
    if request.method == 'POST':
        # form = UserSignUpForm(request.POST or None,request.FILES)
        # if form.is_valid():
        # email = form.cleaned_data['email']
        ...
        if User.objects.filter(email=email).exists():
            context['message']['content'] = 'This email has been already registered.'
            return HttpResponse(
                json.dumps(context),
                content_type="application/json"
                )
        confirm = UserConfirmation(
            email=email, full_name=full_name, password=password, profile_pic=profile_pic)
        # confirm.profile_pic = form.files["profile_pic"]
        confirm.save()
        protocol = 'https' if request.is_secure() else 'http'
        host = request.get_host()
        url = f"{protocol}://{host}/user-signup-verification/{confirm.id}"
        message_type, message_content = send_email(
            to_email=email, url=url, fullname=full_name, mode='sign_up')
...
def user_confirmation(request, confirm_id, backend='django.contrib.auth.backends.ModelBackend'):
    ...
    profile.profile_pic = confirmation.profile_pic
    profile.save()
    UserConfirmation.objects.filter(email=email).delete()
