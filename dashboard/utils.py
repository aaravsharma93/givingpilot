...
class Utils:
    ...
    def get_all_published_campaigns():
        obj = Campaign.objects.select_related('owner').filter(is_published=True)
        if len(obj)!=0:
            for i in range(len(obj)):
                if obj[i].has_end_date:
                    obj[i].days=(obj[i].end_date-datetime.date.today()).days
                    if obj[i].days < 0:
                        obj[i].days = 0
                    print(obj[i].days)
                else :
    ...
    @staticmethod
    def get_campaign_contributors(campaign: Campaign):
        data = Contributor.objects.filter(campaign=campaign)
        # data = data.distinct("user")
        return {
            "data": data,
            "count": data.count()
        }
