...
from payments.views import StripeAuthorizeView
...
def new_campaign(request, campaign_id, create_or_update):
    ...
    try:
        context['seller'] = request.user.seller
    except Seller.DoesNotExist as e:
        context['seller'] = None
    try:
        context['campaign.organization_name'] = campaign.organization_name
        context['campaign.organization_description'] = campaign.organization_description
        context['campaign.organization_logo'] = campaign.organization_logo
    except:
        pass    
...
def new_campaign_process(request, process):
    if request.method == 'POST':
        ...
        if process == 'about_campaign':
            ...
            # campaign.seller = Seller.objects.get(id=request.POST.get('seller_id'))
                ...
                if campaign.large_media:
                    try:
                        os.remove(os.path.join(settings.MEDIA_ROOT,
                                               campaign.large_media.name))
                    except:
                        pass
                if campaign.large_image:
                    try:
                        os.remove(os.path.join(settings.MEDIA_ROOT,
                                               campaign.large_image.replace('media/', '')))
                    except:
                        pass
            ...
            campaign.goal_amount = int(camp_goal_amount)
            campaign.save()
        if process == 'about_you':
            ...
            is_your_email_private = True if request.POST.get(
                'is_your_email_private') == 'true' else False
            is_your_phone_private = True if request.POST.get(
                'is_your_phone_private') == 'true' else False
            is_your_title_private = True if request.POST.get(
                'is_your_title_private') == 'true' else False
        ...
        if process == 'other_options':
            ...
            is_camp_searchable = True if request.POST.get(
                'is_camp_searchable') == 'true' else False
            is_camp_private = True if request.POST.get(
                'is_camp_private') == 'true' else False
...
def get_kpi(request):
    if request.method == "GET":
        days = int(request.GET.get("delta"))
        campaign = request.GET.get("campaign_id")
        contributors = Contributor.objects.filter(campaign_id=campaign,
                                                  created_at__gte=datetime.datetime.now() - datetime.timedelta(
                                                      days=days)) \
            .annotate(day=TruncDay('created_at'), created_count=Count('created_at__date')).values(
            'day',
            'created_count')
        days = []
        data = []
        for contri in contributors:
            days.append(contri["day"].strftime("%-d %b"))
            data.append(contri["created_count"])
...
def small_medias_upload(request):
    if request.method == 'POST':
        ...
        keep_media_ids = [value for key,
                                    value in request.POST.items() if 'media_id_' in key]
        is_created_indices = [key.replace(
            'is_created_', '') for key in request.POST if 'is_created_' in key]
        medias = Media.objects.filter(
            campaign=campaign).exclude(id__in=keep_media_ids)
        for media in medias:
            if media.file:
            ...
            if media.thumbnail:
                t_url = media.thumbnail.replace(
                    Utils.get_protocol_and_host(request), '').replace('media/', '')
                os.remove(os.path.join(settings.MEDIA_ROOT, t_url))
        medias.delete()
        for index in is_created_indices:
            if 'canvas_' + index in request.POST:
                image = base64_to_image(
                    request.POST['canvas_' + index], request.POST['username'])
                thumbnail_path = os.path.join(Utils.get_protocol_and_host(request), 'media', 'thumbnail',
                                              Utils.get_thumbnail(image, media_type='image', is_small=True))
                Media(campaign=campaign, image=image,
                ...
                Media(campaign=campaign, media_url=media_url,
                      thumbnail=thumbnail_path, is_video=True).save()
            else:
