...
$("#category-sd .sidebar-item").click(function (){

    let cat_sections = $(".cat-section");
    for (let sec of cat_sections){
        let disp_attr = sec.style.display;
        if (disp_attr === "block"){
            sec.style.display = "None";
            let category_id = sec.getAttribute("data-key");
            $(`#${category_id}`).removeClass("active");
        }
    }

    const section_id = $(this).attr("data-key");
    let ele = $(`#campaign-${section_id}`)
