...
$('#cropping-image-panel .btn-crop').click(function () {
    ...
    // var filename = original_img.replace(/^.*[\\\/]/, '');
    // fileName = '../../../media/saved_media/'+filename
    // saveBase64AsFile(base64file, fileName)
...
$('#upload-modal .save').click(function () {
    ...
    if (embeddedURL == ""){
        let html = $imgToCrop.cropper('getCroppedCanvas', {maxWidth: 4096, maxHeight: 4096});
        embeddedURL =html.toDataURL("image/png");
    }
...
async function forstripeaccount() {
    protocol = $(location).attr('protocol')
    host = $(location).attr('host')
    const uuid = $('#campaign_id').val();
    url = protocol+'//'+host+'/payment/authorize/'+uuid
...
async function onSaveMiniMedias() {
    const smallMedias = $('.about-campaign .small-media .hidden-media');
    const url = $('.small-medias-save-url').val();
    const username = $('#username').val();
    let formData = new FormData();
...
async function onSavenexAboutCampaign() {
    ...
    let formData = new FormData();
    formData.append('uuid', uuid);
    formData.append('seller_id',$("#seller_id").attr("data-key"));
    formData.append('seller_id',1);
    formData.append('camp_title', $('.camp-title').val());
    formData.append('camp_description', $("#nc-camp-description").val());
    // formData.append('camp_description', tinymce.get('nc-camp-description').getContent());
    formData.append('seller_id',$("#payment-linked").val())
    formData.append('seller_id',1)
...
function onAboutCampaignValidation() {
    ...
    const campaignDescription = $("#nc-camp-description").val();
    ...
    const paymentValue = $("#payment-linked").val();
    const paymentAlert = $("#payment-alert");
    let validProcessNumber = 5;
    ...
    if(isEmptyString(paymentValue)){
        paymentAlert.removeClass('d-none');
    } else {
        if(paymentAlert.hasClass('d-none') === false){
...
if ($('#org-name').val() != '')  {
    $( document ).ready(function() {
        console.log( "ready!" );
        $('#nc-terms-and-conditions').addClass("d-none");
        $('#nc-basic-info').addClass("d-none");
        $('#nc-about-campaign').removeClass("d-none");
        $('.tab-terms-and-conditions').removeClass('unchecked active').addClass('checked')
        $('.tab-basic-info').removeClass('unchecked').addClass('checked')
        $('.tab-about-campaign').addClass('unchecked').addClass('active')
    });
}
