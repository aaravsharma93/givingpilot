...
def get_upload_logo_path(instance, filename):
    current_date = datetime.datetime.now()
    return os.path.join(
        instance.owner.username, current_date.strftime(
            '%m%d%y'), 'logo', filename
    )
...
class Campaign(models.Model):
    ...
    owner = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='campaign')
    ...
    organization_logo = models.FileField(
        upload_to=get_upload_logo_path, default='default-logo.png')

    title = models.CharField(max_length=500, blank=True)
    description = models.TextField(blank=True)
    cause = models.CharField(max_length=500, blank=True)
    large_media = models.FileField(
        upload_to=get_upload_large_media_path, blank=True)
    ...
    goal_amount = models.BigIntegerField(blank=True, default=0)
    ...
    category = models.ForeignKey(
        Category, on_delete=models.CASCADE, related_name='campaign', blank=True, null=True)
    ...
    raised_fund = models.DecimalField(
        default=0.00, max_digits=10, decimal_places=2)
    raised_percentage = models.IntegerField(default=0)
    seller = models.ForeignKey("accounts.Seller", on_delete=models.SET_NULL, blank=True, null=True)

    contributors = models.ManyToManyField(User, related_name="contributors", through="Contributor",
                                          blank=True)
    ...
    def imp_raise_fund(self, fund_amount):
        self.raised_fund += decimal.Decimal(fund_amount)
        self.save()

    def save(self, *args, **kwargs):
        if self.goal_amount:
            self.raised_percentage = float(self.raised_fund) / float(self.goal_amount) * 100
...
def get_upload_media_path(instance, filename):
    current_date = datetime.datetime.now()
    return os.path.join(
        instance.campaign.owner.username, current_date.strftime(
            '%m%d%y'), filename
    )
...
class Manager(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    campaign = models.ForeignKey(
        Campaign, on_delete=models.CASCADE, related_name='manager')
